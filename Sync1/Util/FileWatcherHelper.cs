...
namespace Glasscubes.Drive.Util
{
    ...
    public class FileWatcherHelper : IDirectoryMonitor
    {
        ...
        public FileWatcherHelper(string dirPath)
        {
        ...
        {
            List<string> results = new List<string>(); DateTime now = DateTime.Now; foreach (KeyValuePair<string, DateTime> entry in events)
            {
                // If the path has not received a new event in the last 75ms // an event for the path should be fired 
